...
function checkUserLoginStatus() {
    ...
    if (currentUser && loginBtn) {
        ...
        // Add styles for user menu if not already added
...
async function logoutUser() {
    ...
    // Try clearing server-side currentUser first (json-server expects PUT in this project)
    try {
    ...
    // Clear all possible client-side session keys used across the app
    try {
        localStorage.removeItem('campusConnectCurrentUser');
        localStorage.removeItem('user');
        localStorage.removeItem('currentUser');
        localStorage.removeItem('isLoggedIn');
        // add any other known keys here if needed
    } catch (e) {
        // ignore storage errors
    }

    // Prefer replace so Back button doesn't restore logged-in page
    try {
